...
//this script swaps position of the two objects, aligning centers. Size changes in proportion to height.
    ...
    if(documents.length > 0) {
        if(docRef.selection.length == 2) {
            ...
            if(mySelection instanceof Array) {
                ...
                //scale A (selection[0]) 			
                var factor = B_Height / A_Height;
                mySelection[0].height = Math.abs(B_Height);
                mySelection[0].width = mySelection[0].width * factor;

                //scale B (selection[1])
                var factor2 = A_Height / B_Height;
                mySelection[1].height = Math.abs(A_Height);
                mySelection[1].width = mySelection[1].width * factor2;
